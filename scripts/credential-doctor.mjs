...
export function candidatePaths(home) {
  return [
    {
...
export function buildCredentialDoctorReport(home = os.homedir()) {
  const candidates = candidatePaths(home);
  const report = {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  console.log(JSON.stringify(buildCredentialDoctorReport(), null, 2));
}
