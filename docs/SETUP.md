...
1. **Pair PC:** paste the private host pairing JSON into **Host pairing details**, choose **Read pairing details**, then **Save & connect**. Reading fills the connection fields and clears the paste box; saving encrypts the settings on this PC. Check the PC connection status. The host needs `relayUrl`, `hostId`, `hostToken`, and `ownerToken`; it does not require `botToken` to connect. Individual fields are available under **Advanced connection settings**; leaving a token field blank keeps the last saved secret instead of wiping it.
