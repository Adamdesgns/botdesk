...
export function validateCommand(name, args = {}, context = {}) {
  ...
  if (name === 'click' || name === 'move') {
    if (!inGeometry(args, foreground.geometry)) return reject('bad-arguments', 'Coordinates must fall inside the approved screenshot.');
    if (name === 'click') {
      const button = args.button == null ? 'left' : args.button;
      const count = args.count == null ? 1 : args.count;
  ...
  if (name === 'scroll') {
    ...
    if ((!hasY && !hasX) || (hasY && Math.abs(args.deltaY) > 1200) || (hasX && Math.abs(args.deltaX) > 1200) ||
      ((args.deltaY || 0) === 0 && (args.deltaX || 0) === 0)) {
      return reject('bad-arguments', 'Scroll needs nonzero deltaY and/or deltaX from -1200 to 1200.');
    }
  }
  if (name === 'clipboard_write' && (typeof args.text !== 'string' || args.text.length < 1 || args.text.length > 4000 || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(args.text))) {
    return reject('bad-arguments', 'Clipboard text must be plain printable text, up to 4,000 characters.');
  }
  return { allowed: true };
}
