...
export const TOOL_DEFS = [
  ...
  { name: 'botdesk_list_monitors', description: 'List connected monitors (bounds, working area, primary, device name) for coordinate awareness. Available while armed. Does not include per-monitor scale and does not grant full-desktop capture.', inputSchema: schema() },
  { name: 'botdesk_focus', description: 'Bring only the already-approved target window (its stored HWND and PID) back to the foreground. Never selects a different window. Re-checks integrity, sensitive titles, password controls and the app allowlist. Windows may refuse SetForegroundWindow; that fails closed as focus-refused without picking another target or bypassing UAC.', inputSchema: schema() },
